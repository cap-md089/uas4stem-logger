...
setInterval(function () {
    var npc = packetCount;
    UASSession_1.default.connection.packets += npc;
    UASSession_1.default.connection.packetsPerSecond = npc * (1000 / checkTime);
    packetCount = 0;
}, checkTime);
var server = dgram_1.createSocket('udp4');
server.on('message', function (buff) {
    packetCount += 1;
    var data = buff.toString();
    try {
        data = data.toString();
        var bestData = data;
        var ncs = parseCSV(bestData);
        if ((ncs.throttle > 12 || ncs.groundspeed > 3) &&
            ncs.armed &&
            !UASSession_1.default.flying) {
            UASSession_1.default.prepare();
            ...
            log('--Flight ' +
                ...
                (Math.round(Date.now() / 1000) -
                    start -
                    UASSession_1.default.flightTimer) +
                's, ' +
                (Math.round(Date.now() / 1000) -
                    start -
                    UASSession_1.default.batteryTimer) +
                's total)--');
        }
        currentState = ncs;
    }
    ...
    $('#connection').html("Connection speed: " + ('000' + UASSession_1.default.connection.packetsPerSecond.toString()).substr(-3) + " packet/s");
...
$('#batteryIdSel').change(function () {
    UASSession_1.default.batteryId = parseInt($('#batteryIdSel')
        .find(':selected')
        .text(), 10);
...
{
    var cameraValues = UASSession_1.default.getCameraValues(25);
    $('#cameracalco').html('Width: ' +
        cameraValues.width.toFixed(1) +
        'm; ' +
        'Depth: ' +
...
function update() {
    ...
    var timeRequired = Math.round(currentState.timeRequired);
    ...
    fs.stat(file, function (err1, _) {
        if (err1 && err1.code === 'ENOENT') {
            ...
            sessions[UASSession_1.default.sessionID] = {
                ...
                timeInAir: Math.round(Date.now() / 1000) -
                    start -
                    UASSession_1.default.flightTimer,
...
function stopRecording() {
    ...
    UASSession_1.default.coords.push({
        latitude: lat,
        longitude: lng,
        time: Math.round(Date.now() / 1000) - start - UASSession_1.default.flightTimer,
    ...
    log('"' +
        ...
        lng.toFixed(7) +
        ' (' +
        (Math.round(Date.now() / 1000) - start - UASSession_1.default.flightTimer) +
        ') ' +
        '[' +
        UASSession_1.default.recordingLats.length +
        '/' +
        UASSession_1.default.recordingLongs.length +
        ']');
...
$('#continueFlightButton').on('click', function () {
    $('#continueFlightStatus').text('YES');
    UASSession_1.default.continuingFlight = true;
});
